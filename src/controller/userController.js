  ...
  try {
    ...
    if (!/^\s*[a-zA-Z ]{2,}\s*$/.test(fname)) {
      return res.status(400).send({
        status: false, message: `Heyyy....! ${fname} is not a valid first name`,
      });
    }
    ...
    if (!/^\s*[a-zA-Z ]{2,}\s*$/.test(lname)) {
      return res.status(400).send({
        status: false, message: `Heyyy....! ${lname} is not a valid  last name`,
      });
    }
    ...
    if (!/([/|.|\w|\s|-])*\.(?:jpg|jpeg|png|JPG|JPEG|PNG)/.test(fileData.originalname)) {
      return res.status(400).send({
        status: false, message: "please Add your profile Image with a valid only in JPG JPEG PNG."
      });
    }
    ...
    if (!/^\s*(?=[6789])[0-9]{10}\s*$/.test(phone)) {
      return res.status(400).send({
        status: false, message: `Heyyy....! ${phone} is not a valid phone`,
      });
    }
    let checkPhone = await userModel.findOne({ phone: phone });
    if (checkPhone) {
      return res.status(400).send({
        status: false, message: `Heyyy....! there already exists an account registered with ${phone} phone`,
      });
    }
    ...
    if (!data.address) {
      return res.status(400).send({
        status: false, msg: "please add address details"
      });
    }
    if (!data.address.shipping) {
      return res.status(400).send({
        status: false, msg: "please add shipping details"
      });
    }
    if (!isValidData(data.address.shipping.street) || !/^\s*[a-zA-Z0-9 .,-:]{2,}\s*$/.test(data.address.shipping.street)) {
      return res.status(400).send({
        status: false, msg: "please add valid shipping street"
      });
    }

    if (!isValidData(data.address.shipping.city) || !/^([a-zA-Z\.]+)$/.test(data.address.shipping.city)) {
      return res.status(400).send({
        status: false, msg: "please add valid shipping city"
      });
    }
    ...
    if (!isValidData(data.address.shipping.pincode) || !/^[1-9]{1}[0-9]{2}[0-9]{3}$/.test(data.address.shipping.pincode)
      ...
      return res.status(400).send({
        status: false, msg: "please add valid shipping pincode"
      });
    }

    if (!data.address.billing) {
      return res.status(400).send({
        status: false, msg: "please add   billing details  "
      });
    }

    if (!isValidData(data.address.billing.street) || !/^\s*[a-zA-Z0-9 .,-:]{2,}\s*$/.test(data.address.billing.street)) {
      return res.status(400).send({
        status: false, msg: "please add valid billing street"
      });
    }
    ...
    if (!isValidData(data.address.billing.city) || !/^([a-zA-Z]+)$/.test(data.address.billing.city)
      ...
      return res.status(400).send({
        status: false, msg: "please add valid billing city"
      });
    }
    ...
    if (!isValidData(data.address.billing.pincode) || !/^[1-9]{1}[0-9]{2}[0-9]{3}$/.test(data.address.billing.pincode)
      ...
      return res.status(400).send({
        status: false, msg: "please add valid billing pincode"
      });
    }
    ...
    return res.status(201).send({
      status: true, message: "User created Successfully", data: createUserDoc,
    });
  } catch (err) {
